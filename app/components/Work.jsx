...
const Work = () => {
    return (
        <div id='work' className='w-full px-[10%] py-10 scroll-mt-20 bg-white '>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-6'>
                {
                    workData.map(({ title, description, bgImage, link }, index) => (
                        <a href={link} key={index} target="_blank">
                            <div className='aspect-auto bg-no-repeat bg-cover bg-center rounded-lg relative cursor-pointer group shadow-xl' style={{
                                backgroundImage: `url(${bgImage})`, backgroundSize: 'contain', backgroundPosition: 'center', height: '280px', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center',
